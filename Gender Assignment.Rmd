...
ggplot(heatmap_part2, aes(x = Gender, y = Concept, fill = Percentage)) +
  ...
  scale_fill_gradient(low = "#e0f3db", high = "#4daf4a") +
  labs(
    title = "Gender Proportions per Concept",
    x = NULL,
    y = NULL
  ) +
  theme_minimal() +
...
Secondly, to check differences between categories, we compute entropy of Gender within each ReplaceConcept:
...
ggplot(all_entropy, aes(
  ...
  theme(
    ...
    plot.title = element_text(size = 30, family = "Georgia"),
    axis.title.x = element_text(size = 20, face = "bold", family = "Georgia"), 
    axis.text.x = element_text(size = 16, family = "Georgia"),
    strip.text = element_text(size = 18, face = "bold", family = "Georgia") 
  )
...
ggplot(residuals_df, aes(x = Gender, y = Concept, fill = Residual)) +
  ...
  scale_fill_gradient2(
    low = "#D55E00", mid = "white", high = "#009E73", midpoint = 0,
    name = "Std. Residual"
  ) +
  labs(
...
ggplot(residuals_all, aes(x = Gender, y = ReplaceConcept, fill = Residual)) +
  ...
  geom_text(aes(label = round(Residual, 1)), 
            size = 4, color = "gray30", family = "Georgia") +
...
ggplot(all_residuals, aes(x = Gender, y = Group, fill = Residual)) +
  ...
  geom_text(
    aes(label = round(Residual, 1)),
    size = 8, color = "gray30", family = "Georgia"
  ) +
  scale_fill_gradient2(
...
## Sanity check with inflated dataset
...
```
 ...
 Culture Nature and Animate Sampled data (1000 points per family):
```{r}
#Culture
